...
ggplot(staney_baseflow) +
  ...
  labs(title = "Staney Creek Flow Type",
       ...
       fill = "Flow Type")
```
**Figure 5.** Above is a plot of the flow type for Staney Creek, showing only data from 2022 for clarity. Notice how its data range surpasses 5000 cubic feet per second.
...
ggplot(hatchery_baseflow) +
  ...
  labs(title = "Hatchery Creek Flow Type",
       ...
       fill = "Flow Type")
```
**Figure 7.** Above is a plot of the flow type for Hatchery Creek, showing only data from 2022 for clarity. Notice how its data range doesn't surpass even 2,500 cubic feet per second, less than half of the maximum data points for Staney Creek.
